...
pub enum Error {
    ...
    ShufflesUnsupported,
    #[error(
        "permuted column not found in any *_queries list at Rotation::cur - \
...
pub fn compile_vk(
    ...
    vk: &VerifyingKey<Fr, GwcKZGCommitmentScheme<Bn256>>,
) -> Result<Vec<u8>, Error> {
    let k = params.max_k();
    ...
    for lookup in cs.lookups() {
        ...
        for e in inputs {
            input_bcs.push(encode_expression(e, cs)?);
        }
        let mut table_bcs: Vec<Vec<u8>> = Vec::with_capacity(tables.len());
        for e in tables {
            table_bcs.push(encode_expression(e, cs)?);
        }
        lookup_blobs.push((input_bcs, table_bcs));
    }

    // ── pre-encode shuffle expression bytecodes ─────────────────────────
    // The IOG Halo2 fork used by the shielded-pool circuit has no shuffle
    // argument. Keep the flat VK block present with a zero count.
    let shuffle_blobs: Vec<(Vec<Vec<u8>>, Vec<Vec<u8>>)> = Vec::new();
        ...
        let (col_type, query_index) = match col.column_type() {
            Any::Advice(_) => {
                let idx = cs
                    .advice_queries()
                    .iter()
                    .position(|(c, r)| c.index() == col.index() && r.0 == Rotation::cur().0)
    ...
    for p in fixed_commits {
        out.extend_from_slice(&g1_affine_to_bytes_be(&p.to_affine()));
    }
    out.extend_from_slice(&(perm_commits.len() as u32).to_le_bytes());
    for p in perm_commits {
        out.extend_from_slice(&g1_affine_to_bytes_be(&p.to_affine()));
    }
