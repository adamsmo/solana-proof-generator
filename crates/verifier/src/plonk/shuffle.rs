...
pub fn expressions(
    ...
    for (i, arg) in vk.shuffles.iter().enumerate() {
        single_shuffle_expressions(arg, proof.shuffle_evals[i], ch, lag, active, &ctx, &mut out)?;
    }
    Ok(out)
}
...
fn compute_expr_3(
    ...
    let left = fr_mul(&z_w, &(shuffle_compressed + gamma));
    let right = fr_mul(&z, &(input_compressed + gamma));
