...
pub fn expressions(
    ...
    for (i, arg) in vk.lookups.iter().enumerate() {
        single_lookup_expressions(arg, &proof.lookup_evals[i], ch, lag, active, &ctx, &mut out)?;
    }
    Ok(out)
}
...
fn single_lookup_expressions(
    ...
    // expr_1: l_0 · (1 − z)
    out.push(fr_mul(&lag.l_0, &(Fr::ONE - z)));
    // expr_2: l_last · (z² − z)
    out.push(fr_mul(&lag.l_last, &(fr_square(&z) - z)));
    // expr_3: active · (left − right) - heaviest Fr arithmetic, isolated frame.
    out.push(compute_expr_3(
        evals,
        ch,
        active,
        input_compressed,
    ...
    // expr_4: l_0 · (a' − s')
    out.push(fr_mul(&lag.l_0, &(a_p - s_p)));
    // expr_5: active · (a' − s') · (a' − a'_prev)
    let permuted_difference = a_p - s_p;
    let previous_difference = a_p - a_m;
    out.push(fr_mul(
        &fr_mul(&active, &permuted_difference),
        &previous_difference,
    ));
    Ok(())
}
...
fn compute_expr_3(
    ...
    let left = fr_mul(
        &fr_mul(
            &evals.product_next_eval,
            &(evals.permuted_input_eval + ch.beta),
        ),
        &(evals.permuted_table_eval + ch.gamma),
    );
    let right = fr_mul(
        &fr_mul(&evals.product_eval, &(input_compressed + ch.beta)),
        &(table_compressed + ch.gamma),
    );
    fr_mul(&active, &(left - right))
}
...
mod tests {
    ...
    fn empty_lookups_returns_empty() {
        ...
        let proof = synth_proof(LookupEvals {
            product_eval: Fr::ZERO,
            product_next_eval: Fr::ZERO,
    ...
    fn compress_two_expressions_horner() {
        ...
        // Expected: 2·5 + 3 = 13.
        let r = compress(
            &alloc::vec![advice_bc(0), advice_bc(1)],
            Fr::from(5u64),
            &ctx,
        )
        .unwrap();
