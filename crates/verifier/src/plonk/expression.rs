...
pub fn evaluate(bytecode: &[u8], ctx: &EvalContext<'_>) -> Result<Fr, Error> {
    ...
    while cur < bytecode.len() {
        ...
        match op {
            ...
            OP_FIXED => {
                let idx = read_u32(bytecode, &mut cur)? as usize;
                let v = ctx.fixed_evals.get(idx).ok_or(Error::InvalidGateBytecode)?;
                stack.push(*v);
            }
            OP_INSTANCE => {
                let idx = read_u32(bytecode, &mut cur)? as usize;
            ...
            OP_MUL => {
                let a = stack.pop().ok_or(Error::InvalidGateBytecode)?;
                let b = stack.pop().ok_or(Error::InvalidGateBytecode)?;
                stack.push(fr_mul(&b, &a));
            }
            OP_SCALE => {
                let bytes = read_32(bytecode, &mut cur)?;
                let c = fr_from_bytes_be(&bytes)?;
                let a = stack.pop().ok_or(Error::InvalidGateBytecode)?;
                stack.push(fr_mul(&a, &c));
            }
...
mod tests {
    ...
    fn standard_plonk_gate() {
        ...
        let mut bc = Vec::new();
        push_fixed(&mut bc, 0);
        push_advice(&mut bc, 0);
        bc.push(OP_MUL); // q_a*a
        push_fixed(&mut bc, 1);
        push_advice(&mut bc, 1);
        bc.push(OP_MUL); // q_b*b
        bc.push(OP_ADD); // +
        push_fixed(&mut bc, 2);
        push_advice(&mut bc, 2);
        bc.push(OP_MUL); // q_c*c
        bc.push(OP_ADD); // +
        push_fixed(&mut bc, 3);
        push_advice(&mut bc, 0);
        push_advice(&mut bc, 1);
