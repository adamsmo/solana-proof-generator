...
pub fn expressions(
    ...
    out.push(field::fr_mul(&lag.l_0, &(Fr::ONE - first.0)));

    // (2) L_last(x) · (z_l(x)² − z_l(x)) - last set final.
    let last = &proof.permutation_product_evals[vk.num_perm_chunks - 1];
    let last_squared = field::fr_square(&last.0);
    out.push(field::fr_mul(&lag.l_last, &(last_squared - last.0)));
    ...
    for i in 1..vk.num_perm_chunks {
        ...
        out.push(field::fr_mul(&lag.l_0, &(z_i - z_im1_last)));
    }
    ...
    for i in 0..vk.num_perm_chunks {
        let contribution = chunk_grand_product(
            ...
            &proof.fixed_evals,
            instance_evals,
            &proof.permutation_common_evals,
            &proof.permutation_product_evals,
            ch.beta,
            ch.x,
            ch.gamma,
            delta,
        )?;
        out.push(field::fr_mul(&active, &contribution));
    }
...
fn chunk_grand_product(
    ...
    for j in col_start..col_end {
        ...
        let beta_sigma = field::fr_mul(&beta, &perm_common_evals[j]);
        left = field::fr_mul(&left, &(col_eval + beta_sigma + gamma));
    }

    // right = z(x) · ∏ (column_eval + δⁱ · β · x + γ)
    let delta_power = field::fr_pow_u64(&delta, col_start as u64);
    let beta_x = field::fr_mul(&beta, &x);
    let mut current_delta = field::fr_mul(&beta_x, &delta_power);
    let mut right = z;
    for j in col_start..col_end {
        let col_eval = lookup_column_eval(
            permuted_columns[j],
        ...
        right = field::fr_mul(&right, &(col_eval + current_delta + gamma));
        current_delta = field::fr_mul(&current_delta, &delta);
    }
    Ok(left - right)
}
...
mod tests {
    ...
    fn synth_vk(
        num_advice: usize,
        num_perm_columns: usize,
        cs_degree: usize,
        num_perm_chunks: usize,
    ) -> PlonkProtocol {
    ...
    fn synth_proof_one_chunk(num_advice: usize, num_perm_cols: usize) -> PlonkProof {
        PlonkProof {
            ...
            advice_evals: (0..num_advice)
                .map(|i| Fr::from((i as u64) * 100 + 1))
                .collect(),
            fixed_evals: Vec::new(),
            ...
            permutation_common_evals: (0..num_perm_cols)
                ...
                .collect(),
            permutation_product_evals: alloc::vec![(
                Fr::from(31u64),
                Fr::from(37u64),
                Fr::from(41u64)
            )],
