...
use crate::{
    curve::G1,
    field::{fr_inverse, fr_mul, fr_pow_u64, fr_to_bytes_be},
    kzg::{gwc, shplonk, shplonk::VerifierQuery, KzgVk},
    pairing,
    plonk::{
        expression, lagrange, lookup, permutation, shuffle, Challenges, PlonkProof, PlonkProtocol,
    },
    proof_reader,
    transcript::{IogKeccak256Transcript, Keccak256Transcript},
    vk::parse_vk,
    Error,
};
...
pub fn compute_expected_h_eval(
    ...
    expressions.extend(lookup::expressions(
        vk,
        proof,
        ch,
        lag,
    ...
    expressions.extend(shuffle::expressions(
        vk,
        proof,
        ch,
        lag,
    ...
    let folded = expressions
        .iter()
        .fold(Fr::ZERO, |acc, e| fr_mul(&acc, &ch.y) + e);
    let xn_inv = fr_inverse(&(lag.xn - Fr::ONE))
        .ok_or(Error::Protocol("verify: xn − 1 = 0 (x on subgroup)"))?;
    let expected = fr_mul(&folded, &xn_inv);
...
pub fn build_queries(
    ...
    let perm_prod_ids: Vec<usize> = proof
    ...
    // set per commit_id.
    let lookup_product_ids: Vec<usize> =
        proof.lookup_product_commits.iter().map(|_| id()).collect();
    let lookup_permuted_input_ids: Vec<usize> = proof
        .lookup_permuted_input_commits
        .iter()
        .map(|_| id())
        .collect();
    let lookup_permuted_table_ids: Vec<usize> = proof
        .lookup_permuted_table_commits
        .iter()
        .map(|_| id())
        .collect();
    // v2.0 shuffle commit IDs - 1 product commit per shuffle, queried at x + xω.
    let shuffle_product_ids: Vec<usize> =
        proof.shuffle_product_commits.iter().map(|_| id()).collect();
    ...
    for (q_idx, (col_index, rotation)) in vk.advice_queries.iter().enumerate() {
        ...
        let commit = *proof.advice_commits.get(col).ok_or(Error::Protocol(
            "build_queries: advice column index out of range",
        ))?;
        let point = rotate_point(ch.x, omega, *rotation, n);
        let eval = *proof.advice_evals.get(q_idx).ok_or(Error::Protocol(
            "build_queries: advice eval index out of range",
        ))?;
        q.push(VerifierQuery {
            commit_id: advice_col_ids[col],
            commitment: commit,
            point,
            eval,
        });
    ...
    let x_next = fr_mul(&ch.x, &omega);
    let x_last = fr_mul(&ch.x, &omega_last);
    for ((cid, commit), evals) in perm_prod_ids
        .iter()
        ...
        || vk.num_lookups() != proof.lookup_permuted_table_commits.len()
    {
        return Err(Error::Protocol(
            "build_queries: lookup commit/eval count mismatch",
        ));
    }
    if vk.num_lookups() > 0 {
        ...
        for i in 0..vk.num_lookups() {
            ...
            q.push(VerifierQuery {
                commit_id: lookup_product_ids[i],
                commitment: p_commit,
                point: ch.x,
                eval: evals.product_eval,
            });
            q.push(VerifierQuery {
                commit_id: lookup_permuted_input_ids[i],
                commitment: pi_commit,
                point: ch.x,
                eval: evals.permuted_input_eval,
            });
            q.push(VerifierQuery {
                commit_id: lookup_permuted_table_ids[i],
                commitment: pt_commit,
                point: ch.x,
                eval: evals.permuted_table_eval,
            });
            q.push(VerifierQuery {
                commit_id: lookup_permuted_input_ids[i],
                commitment: pi_commit,
                point: x_prev,
                eval: evals.permuted_input_inv_eval,
            });
            q.push(VerifierQuery {
                commit_id: lookup_product_ids[i],
                commitment: p_commit,
                point: x_next,
                eval: evals.product_next_eval,
            });
        }
    }
    ...
    {
        return Err(Error::Protocol(
            "build_queries: shuffle commit/eval count mismatch",
        ));
    }
    for i in 0..vk.num_shuffles() {
        ...
        q.push(VerifierQuery {
            commit_id: shuffle_product_ids[i],
            commitment: commit,
            point: ch.x,
            eval: z,
        });
    ...
    for (q_idx, (col_index, rotation)) in vk.fixed_queries.iter().enumerate() {
        let col = *col_index as usize;
        let commit = *vk.fixed_commitments.get(col).ok_or(Error::Protocol(
            "build_queries: fixed column index out of range",
        ))?;
        let point = rotate_point(ch.x, omega, *rotation, n);
        let eval = *proof.fixed_evals.get(q_idx).ok_or(Error::Protocol(
            "build_queries: fixed eval index out of range",
        ))?;
        q.push(VerifierQuery {
            commit_id: fixed_col_ids[col],
            commitment: commit,
            point,
            eval,
        });
    ...
    q.push(VerifierQuery {
        ...
        point: ch.x,
        eval: expected_h_eval,
    });
    q.push(VerifierQuery {
        commit_id: random_id,
        commitment: proof.random_poly_commit,
        point: ch.x,
        eval: proof.random_poly_eval,
    });
...
fn rotate_point(x: Fr, omega: Fr, rotation: i32, n: u64) -> Fr {
    ...
    fr_mul(&x, &fr_pow_u64(&omega, exp))
}

#[inline(never)]
fn prepare_opening_queries(
    vk: &PlonkProtocol,
    proof: &PlonkProof,
    ch: &Challenges,
    public_inputs: &[[u8; 32]],
    ...
    let public_inputs_per_column: Vec<Vec<Fr>> = if vk.num_instance == 0 {
        Vec::new()
    } else {
        let mut col0 = Vec::with_capacity(public_inputs.len());
        for raw in public_inputs {
            col0.push(crate::field::fr_from_bytes_be(raw)?);
        }
        let mut columns = Vec::with_capacity(vk.num_instance);
        columns.push(col0);
    ...
    let instance_evals = lagrange::reconstruct_instance_evals(
        ...
        ch.x,
        &vk.instance_queries,
        &public_inputs_per_column,
    )?;
    let expected_h_eval =
        compute_expected_h_eval(vk, proof, ch, &lag, &instance_evals, &ch.user_challenges)?;
    ...
    let omega_last = fr_pow_u64(&vk.omega, last_pow);
...
pub fn verify(
    ...
    #[cfg(feature = "debug-trace")]
    {
        ...
        eprintln!(
            "[verifier] random_poly_eval = {}",
            _fr_hex(&proof.random_poly_eval)
        );
    }

    let queries = prepare_opening_queries(&vk, &proof, &ch, public_inputs)?;
    ...
    #[cfg(feature = "debug-trace")]
    {
        ...
        for (i, q) in queries.iter().enumerate() {
            eprintln!(
                "[verifier] query[{i:>2}]: point={} eval={} commit={}",
                _fr_hex(&q.point),
                _fr_hex(&q.eval),
                _g1_hex(&q.commitment)
            );
        }
    }
...
/// Verify an IOG Halo2 BN254/KZG/GWC proof.
pub fn verify_gwc(
    vk_bytes: &[u8],
    proof_bytes: &[u8],
    public_inputs: &[[u8; 32]],
    ...
    let vk = parse_vk(vk_bytes)?;
    let mut transcript = IogKeccak256Transcript::new(&vk.transcript_repr);
    let (proof, challenges, opening) =
        proof_reader::read_gwc_proof(&vk, proof_bytes, public_inputs, &mut transcript)?;
    let queries = prepare_opening_queries(&vk, &proof, &challenges, public_inputs)?;
    let pairs = gwc::verify_opening(&queries, &opening.witnesses, opening.v, opening.u, kzg_vk)?;
    pairing::pairing_check(&pairs.0)
}
...
mod tests {
    ...
    fn build_queries_count_matches_layout() {
        ...
        let qs =
            build_queries(&vk, &proof, &ch, G1::IDENTITY, Fr::ZERO, vk.omega, Fr::ONE).unwrap();
