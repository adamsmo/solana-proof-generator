...
use crate::{
    ...
    transcript::{IogKeccak256Transcript, Keccak256Transcript, ProofTranscript},
    Error,
};

/// GWC multi-opening proof data read after the common PLONK proof prefix.
#[derive(Clone, Debug)]
pub struct GwcOpeningProof {
    pub v: Fr,
    pub u: Fr,
...
pub fn read_proof(
    ...
    let (proof, challenges, opening) = read_proof_inner(
        vk,
        proof_bytes,
        public_inputs,
        transcript,
...
/// Parse an IOG Halo2 GWC proof using the cumulative, prefixed Keccak
/// transcript used by the Solana proof generator.
pub fn read_gwc_proof(
    vk: &PlonkProtocol,
    proof_bytes: &[u8],
    public_inputs: &[[u8; 32]],
    transcript: &mut IogKeccak256Transcript,
) -> Result<(PlonkProof, Challenges, GwcOpeningProof), Error> {
    let (proof, challenges, opening) = read_proof_inner(
        vk,
        proof_bytes,
        public_inputs,
    ...
    let opening = opening.ok_or(Error::Protocol("read_gwc_proof: missing GWC opening"))?;
    Ok((proof, challenges, opening))
}

fn read_proof_inner<T: ProofTranscript>(
    vk: &PlonkProtocol,
    proof_bytes: &[u8],
    public_inputs: &[[u8; 32]],
    transcript: &mut T,
    multiopen: MultiOpenScheme,
) -> Result<(PlonkProof, Challenges, Option<GwcOpeningProof>), Error> {
    // ── absorb public inputs ───────────────────────────────────────────────
    // IOG Halo2 binds each instance column's length before its values.
    // The flat verifier API supports one populated column and treats any
    // remaining instance columns as empty, matching `prepare_opening_queries`.
    if matches!(multiopen, MultiOpenScheme::Gwc) {
        for column in 0..vk.num_instance {
            let len = if column == 0 { public_inputs.len() } else { 0 };
            let len_be = crate::field::fr_to_bytes_be(&Fr::from(len as u64));
            transcript.absorb_scalar(&len_be);
        }
    }
    ...
    let mut user_challenges = alloc::vec![Fr::from(0u64); vk.num_challenges];
    {
        ...
        let single_phase_legacy = vk.num_phases <= 1
        ...
        if single_phase_legacy {
            ...
            if vk.advice_column_phase.len() != vk.num_advice {
                return Err(Error::Protocol(
                    "read_proof: advice_column_phase length mismatch",
                ));
            }
            if vk.challenge_phase.len() != vk.num_challenges {
                return Err(Error::Protocol(
                    "read_proof: challenge_phase length mismatch",
                ));
            }
    ...
    let lookup_product_commits = read_g1s(transcript, proof_bytes, &mut cur, vk.num_lookups())?;
    let shuffle_product_commits = read_g1s(transcript, proof_bytes, &mut cur, vk.num_shuffles())?;
    ...
    let (shplonk_y, shplonk_v, shplonk_u, opening_proof_w, opening_proof_w_prime, gwc_opening) =
        match multiopen {
            MultiOpenScheme::Shplonk => {
                // Order matches PSE Halo2 SHPLONK:
            ...
            MultiOpenScheme::Gwc => {
                // Order matches IOG Halo2 GWC:
                // squeeze v, read one witness per distinct evaluation point,
                // then squeeze u.
                let v = transcript.squeeze_challenge();
                let witnesses = read_g1s(
                    transcript,
                    proof_bytes,
                    &mut cur,
                    expected_gwc_witness_count(vk),
                )?;
                ...
                (
                    ...
                    G1::IDENTITY,
                    G1::IDENTITY,
                    Some(GwcOpeningProof { v, u, witnesses }),
                )
            }
        };
...
pub fn parse_proof_no_fs(vk: &PlonkProtocol, proof_bytes: &[u8]) -> Result<PlonkProof, Error> {
    let mut cur = 0usize;
    ...
    let single_phase_legacy = vk.num_phases <= 1 && vk.advice_column_phase.is_empty();
    let mut advice_commits = alloc::vec![crate::curve::G1([0u8; 64]); vk.num_advice];
    if single_phase_legacy {
        for c in 0..vk.num_advice {
        ...
        if vk.advice_column_phase.len() != vk.num_advice {
            return Err(Error::Protocol(
                "parse_proof_no_fs: advice_column_phase length mismatch",
            ));
        }
        for p in 0..vk.num_phases {
            for (c, &col_phase) in vk.advice_column_phase.iter().enumerate() {
    ...
    // (2) permutation product commits
    let permutation_product_commits = read_g1s_no_fs(proof_bytes, &mut cur, vk.num_perm_chunks)?;

    // (2.5) per-lookup grand-product commits + per-shuffle product commits
    let lookup_product_commits = read_g1s_no_fs(proof_bytes, &mut cur, vk.num_lookups())?;
    let shuffle_product_commits = read_g1s_no_fs(proof_bytes, &mut cur, vk.num_shuffles())?;
...
#[inline]
fn read_scalars_no_fs(proof: &[u8], cursor: &mut usize, count: usize) -> Result<Vec<Fr>, Error> {
    let mut out = Vec::with_capacity(count);
    for _ in 0..count {
...
#[inline]
fn read_scalars<T: ProofTranscript>(
    transcript: &mut T,
    proof: &[u8],
    cursor: &mut usize,
    count: usize,
...
fn common_proof_element_counts(vk: &PlonkProtocol) -> (usize, usize) {
    let g1_count = vk.num_advice                       // advice commits
    ...
    let fr_count = vk.num_advice_queries
...
pub fn expected_proof_size(vk: &PlonkProtocol) -> usize {
    const G1_UNCOMPRESSED_LEN: usize = 64;
    const FR_LEN: usize = 32;

    let (common_g1_count, fr_count) = common_proof_element_counts(vk);
    let shplonk_opening_count = 2;
    (common_g1_count + shplonk_opening_count) * G1_UNCOMPRESSED_LEN + fr_count * FR_LEN
}

/// Exact byte length of an IOG GWC proof using Solana's 32-byte compressed
/// BN254 G1 encoding.
pub fn expected_gwc_proof_size(vk: &PlonkProtocol) -> usize {
    const G1_COMPRESSED_LEN: usize = 32;
    const FR_LEN: usize = 32;

    let (common_g1_count, fr_count) = common_proof_element_counts(vk);
    let gwc_witness_count = expected_gwc_witness_count(vk);
    (common_g1_count + gwc_witness_count) * G1_COMPRESSED_LEN + fr_count * FR_LEN
}

/// GWC writes one witness commitment for every distinct evaluation point.
/// All points used by PLONK are rotations of the same challenge `x`, so the
/// count can be derived from the VK rotations before `x` is known.
pub fn expected_gwc_witness_count(vk: &PlonkProtocol) -> usize {
    let n = 1u64 << vk.k;
    let mut rotations: Vec<u64> = Vec::new();
    let mut push_rotation = |rotation: i32| {
        let normalized = if rotation >= 0 {
            rotation as u64 % n
        } else {
            let magnitude = (-(rotation as i64)) as u64 % n;
            if magnitude == 0 {
                0
            } else {
                n - magnitude
    ...
    for (_, rotation) in &vk.advice_queries {
        push_rotation(*rotation);
    }
    if vk.num_perm_chunks > 0 {
        push_rotation(0);
        push_rotation(1);
        if vk.num_perm_chunks > 1 {
            push_rotation(-(vk.blinding_factors as i32 + 1));
        }
    }
    if vk.num_lookups() > 0 {
        push_rotation(0);
    ...
    for (_, rotation) in &vk.fixed_queries {
        push_rotation(*rotation);
    }
    if vk.num_perm_columns() > 0 {
        push_rotation(0);
    }
    // Aggregated h and random polynomial are always opened at x.
    push_rotation(0);
...
mod tests {
    ...
    fn expected_proof_size_matches_layout() {
        ...
        // GWC uses 32-byte compressed G1. This synthetic VK opens at x and
        // omega*x, so it has two GWC witnesses:
        // G1 = 8 common + 2 witnesses = 10 → 320 B
        // Fr = 14 → 448 B
        assert_eq!(expected_gwc_proof_size(&vk), 768);
    }
