...
use crate::{
    field::{fr_inverse, fr_mul, fr_pow_u64},
    Error,
};
...
pub fn evaluate_lagrange(
    ...
    let n_inv =
        fr_inverse(&Fr::from(n_u64)).ok_or(Error::Protocol("evaluate_lagrange: n inverse"))?;

    // xn = x^n - domain vanishing polynomial Z_H(x) = xn − 1.
    let xn = fr_pow_u64(&x, n_u64);
    let xn_minus_one = xn - Fr::ONE;

    // Common factor: (xn − 1) · n⁻¹.
    let factor = fr_mul(&xn_minus_one, &n_inv);

    // L_0(x) = (xn − 1) / (n · (x − 1))   (ω⁰ = 1 cancels)
    let denom_0 = fr_inverse(&(x - Fr::ONE)).ok_or(Error::Protocol("evaluate_lagrange: x = 1"))?;
    let l_0 = fr_mul(&factor, &denom_0);

    // ω⁻¹ - used to walk negative indices.
    let omega_inv =
        fr_inverse(&omega).ok_or(Error::Protocol("evaluate_lagrange: omega inverse"))?;
    ...
    let omega_inv_pow_last = fr_mul(&omega_inv_pow_at_blinding, &omega_inv);
    let denom_last = fr_inverse(&(x - omega_inv_pow_last)).ok_or(Error::Protocol(
        "evaluate_lagrange: x = ω⁻(blinding+1) (l_last)",
    ))?;
    let l_last = fr_mul(&fr_mul(&omega_inv_pow_last, &factor), &denom_last);
...
fn accumulate_blinded_lagrange(
    ...
    for _ in 1..=blinding_factors {
        omega_inv_pow_i = fr_mul(&omega_inv_pow_i, &omega_inv);
        let denom = fr_inverse(&(x - omega_inv_pow_i))
            .ok_or(Error::Protocol("evaluate_lagrange: x = ω⁻ⁱ (l_blind)"))?;
        l_blind += fr_mul(&fr_mul(&omega_inv_pow_i, &factor), &denom);
    }
...
pub fn lagrange_basis_at(
    ...
    let omega_pow_i = if i >= 0 {
        fr_pow_u64(&omega, i as u64)
    } else {
        let omega_inv =
            fr_inverse(&omega).ok_or(Error::Protocol("lagrange_basis_at: omega inverse"))?;
        fr_pow_u64(&omega_inv, (-i) as u64)
    };
    let denom = fr_inverse(&(x - omega_pow_i)).ok_or(Error::Protocol(
        "lagrange_basis_at: x equals ω^i (domain point)",
    ))?;
    Ok(fr_mul(
        &fr_mul(&fr_mul(&omega_pow_i, &xn_minus_one), &n_inv),
        &denom,
    ))
}
...
pub fn reconstruct_instance_evals(
    ...
    let n_u64: u64 = 1u64 << k;
    let n_inv = fr_inverse(&Fr::from(n_u64))
    ...
    for (col_index, rotation) in instance_queries {
        let column_values = public_inputs
            .get(*col_index as usize)
            .ok_or(Error::Protocol(
                "reconstruct_instance_evals: column out of range",
            ))?;
        let mut acc = Fr::ZERO;
        for (j, value) in column_values.iter().enumerate() {
            ...
            let basis =
                lagrange_basis_at(j as i64 - *rotation as i64, omega, x, xn_minus_one, n_inv)?;
            acc += fr_mul(value, &basis);
        }
        out.push(acc);
    }
    Ok(out)
...
mod tests {
    ...
    fn xn_formula_correct() {
        ...
        let xn_actual = fr_pow_u64(&x, 1 << 3);
