...
//!
//! Reference: `halo2_proofs/src/poly/kzg/multiopen/shplonk/verifier.rs`.
//! This implementation specialises the algorithm to BN254 and arkworks Fr.
//!
...
#[inline(never)]
pub(crate) fn lagrange_basis_compute(points: &[Fr]) -> Result<(Vec<Vec<Fr>>, Vec<Fr>), Error> {
    let n = points.len();
...
pub(crate) fn construct_intermediate_sets(queries: &[VerifierQuery]) -> IntermediateSets {
    ...
    for q in queries {
        if let Some(pc) = per_commitment
            .iter_mut()
            .find(|pc| pc.commit_id == q.commit_id)
        {
    ...
    for pc in per_commitment {
        let evals_in_point_order: Vec<Fr> = pc
            .points
            .iter()
            .map(|p| {
                pc.evals_by_point
        ...
        if let Some(existing) = rotation_sets.iter_mut().find(|rs| rs.points == pc.points) {
            existing
                .commitments_with_evals
                .push((pc.commitment, evals_in_point_order));
        } else {
...
pub fn verify_opening(
    ...
    let msm_terms = build_shplonk_msm_terms(queries, h1, h2, y, v, u, kzg_vk.g1_one)?;
...
fn compute_z_diff_i(
    ...
    let diffs: Vec<Fr> = super_point_set
        .iter()
...
#[inline(never)]
pub(crate) fn msm_g1(terms: &[(Fr, G1)]) -> Result<G1, Error> {
...
#[inline(never)]
pub(crate) fn neg_g1(p: &G1) -> Result<G1, Error> {
    if p == &G1::IDENTITY {
...
pub fn build_shplonk_msm_terms(
    ...
    eprintln!("[verifier] SHPLONK MSM length = {} terms ({} commitments + 3 finalization); rotation_sets={}, distinct_points={}",
        outer_msm_terms.len(), outer_msm_terms.len() - 3,
        rotation_sets.len(), super_point_set.len());
...
mod tests {
    ...
    fn eval_polynomial_constant() {
        assert_eq!(
            eval_polynomial(&[Fr::from(7u64)], Fr::from(99u64)),
            Fr::from(7u64)
        );
    }
    ...
    fn intermediate_distinct_point_sets_make_distinct_rotation_sets() {
        ...
        assert_eq!(
            sets.super_point_set,
            alloc::vec![Fr::from(7u64), Fr::from(8u64)]
        );
    }
    ...
    fn cached_basis_matches_legacy_lagrange() {
        let points: Vec<Fr> = [1u64, 2u64, 5u64, 11u64]
            .into_iter()
            .map(Fr::from)
            .collect();
        let values: Vec<Fr> = [7u64, 14u64, 35u64, 88u64]
            .into_iter()
            .map(Fr::from)
            .collect();
