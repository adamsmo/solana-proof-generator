...
mod tests {
    ...
    fn wrong_magic_rejects() {
        ...
        bytes[0] = b'X';
        assert!(matches!(
            Stage1Output::deserialize(&bytes),
            Err(Error::InvalidStageState)
        ));
    }
    ...
    fn synth_stage2() -> Stage2Output {
        Stage2Output {
            msm_terms: alloc::vec![synth_term(1, 100), synth_term(2, 101), synth_term(3, 102),],
    ...
    fn stage2_roundtrip_full() {
        ...
        assert_eq!(
            restored.opening_proof_w_prime.0,
            original.opening_proof_w_prime.0
        );
    ...
    fn stage2_wrong_magic_rejects() {
        ...
        bytes[0] = b'X';
        assert!(matches!(
            Stage2Output::deserialize(&bytes),
            Err(Error::InvalidStageState)
        ));
    }
    ...
    fn stage2_oversized_terms_rejects_serialize() {
        ...
        s.msm_terms = (0..(MAX_MSM_TERMS + 1))
            .map(|i| synth_term(0, i as u64))
            .collect();
        let mut bytes = Vec::new();
        assert!(matches!(
            s.serialize(&mut bytes),
