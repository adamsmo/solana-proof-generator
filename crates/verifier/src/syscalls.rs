...
//!
//! The default path uses big-endian 32-byte field elements.
...
mod onchain {
    ...
    /// Decompress Solana's canonical 32-byte big-endian BN254 G1 encoding
    /// into the verifier's internal 64-byte `x || y` big-endian form.
    pub fn g1_decompress(compressed: &[u8; 32]) -> Result<[u8; 64], Error> {
        alt_bn128_g1_decompress_be(compressed).map_err(|error| Error::SyscallFailed {
            which: "alt_bn128_g1_decompress",
            code: error.into(),
        })
    }
    ...
    pub fn g1_add(a: &[u8; 64], b: &[u8; 64]) -> Result<[u8; 64], Error> {
        ...
        let out = {
            ...
            let le_out = alt_bn128_g1_addition_le(&le_input).map_err(|e| Error::SyscallFailed {
                which: "alt_bn128_g1_add_le",
                code: e.into(),
            })?;
        ...
        #[cfg(not(feature = "simd-0284-le"))]
        let out = {
            let raw = alt_bn128_g1_addition_be(&input).map_err(|e| Error::SyscallFailed {
                which: "alt_bn128_g1_add",
                code: e.into(),
            })?;
    ...
    pub fn g1_mul(p: &[u8; 64], scalar_be: &[u8; 32]) -> Result<[u8; 64], Error> {
        ...
        let out = {
            ...
            let le_out =
                alt_bn128_g1_multiplication_le(&le_input).map_err(|e| Error::SyscallFailed {
                    which: "alt_bn128_g1_mul_le",
                    code: e.into(),
                })?;
            let mut le_arr = [0u8; 64];
            le_arr.copy_from_slice(&le_out);
        ...
        let out = {
            ...
            let raw = alt_bn128_g1_multiplication_be(&input).map_err(|e| Error::SyscallFailed {
                which: "alt_bn128_g1_mul",
                code: e.into(),
            })?;
            debug_assert_eq!(raw.len(), 64);
            let mut result = [0u8; 64];
    ...
    pub fn pairing_check(pairs: &[u8]) -> Result<bool, Error> {
        if pairs.is_empty() || pairs.len() % 192 != 0 {
            return Err(Error::Protocol(
                "pairing_check: input not a multiple of 192",
            ));
        }
        #[cfg(feature = "simd-0284-le")]
        let out = {
            ...
            alt_bn128_pairing_le(&le_pairs).map_err(|e| Error::SyscallFailed {
                which: "alt_bn128_pairing_le",
                code: e.into(),
            })?
        };
        #[cfg(not(feature = "simd-0284-le"))]
        let out = alt_bn128_pairing_be(pairs).map_err(|e| Error::SyscallFailed {
            which: "alt_bn128_pairing",
            code: e.into(),
        })?;
    ...
    pub fn g2_add(a: &[u8; 128], b: &[u8; 128]) -> Result<[u8; 128], Error> {
        ...
        let out = alt_bn128_g2_addition_be(&input).map_err(|e| Error::SyscallFailed {
            which: "alt_bn128_g2_add",
            code: e.into(),
        })?;
    ...
    pub fn g2_mul(p: &[u8; 128], scalar_be: &[u8; 32]) -> Result<[u8; 128], Error> {
        ...
        let out = alt_bn128_g2_multiplication_be(&input).map_err(|e| Error::SyscallFailed {
            which: "alt_bn128_g2_mul",
            code: e.into(),
        })?;
        debug_assert_eq!(out.len(), 128);
        let mut result = [0u8; 128];
    ...
    pub fn g2_add(_a: &[u8; 128], _b: &[u8; 128]) -> Result<[u8; 128], Error> {
        ...
        Err(Error::Protocol(
            "g2_add requires SIMD-0302 (devnet) - mainnet shim TBD",
        ))
    }
    ...
    pub fn g2_mul(_p: &[u8; 128], _scalar_be: &[u8; 32]) -> Result<[u8; 128], Error> {
        Err(Error::Protocol(
            "g2_mul requires SIMD-0302 (devnet) - mainnet shim TBD",
        ))
    }
}
...
mod onchain {
    ...
    fn parse_g1_be(bytes: &[u8; 64]) -> Result<G1Affine, Error> {
        ...
        let mut le = [0u8; 64];
        for (dst, src) in le[..32].iter_mut().zip(bytes[..32].iter().rev()) {
            *dst = *src;
        }
        for (dst, src) in le[32..].iter_mut().zip(bytes[32..].iter().rev()) {
            *dst = *src;
    ...
    fn emit_g1_be(p: G1Affine) -> [u8; 64] {
        ...
        x.serialize_with_mode(&mut out_le[..32], Compress::No)
            .unwrap();
        y.serialize_with_mode(&mut out_le[32..], Compress::No)
            .unwrap();
        let mut be = [0u8; 64];
        for (i, b) in out_le[..32].iter().rev().enumerate() {
            be[i] = *b;
        }
        for (i, b) in out_le[32..].iter().rev().enumerate() {
            be[32 + i] = *b;
    ...
    pub fn g1_decompress(compressed: &[u8; 32]) -> Result<[u8; 64], Error> {
        ...
        let mut little_endian = *compressed;
        little_endian.reverse();
        let point =
            G1Affine::deserialize_with_mode(&little_endian[..], Compress::Yes, Validate::No)
                .map_err(|_| Error::Protocol("host g1 decompress"))?;
        Ok(emit_g1_be(point))
    }
    ...
    pub fn g2_add(_a: &[u8; 128], _b: &[u8; 128]) -> Result<[u8; 128], Error> {
        Err(Error::Protocol(
            "g2_add: arkworks fallback TODO (needed only for tests)",
        ))
    }
    pub fn g2_mul(_p: &[u8; 128], _scalar_be: &[u8; 32]) -> Result<[u8; 128], Error> {
        Err(Error::Protocol(
            "g2_mul: arkworks fallback TODO (needed only for tests)",
        ))
    }
}
...
// the `simd-0284-le` cfg branch when that feature is enabled, against an
...
mod a2_tests {
    ...
    fn fr_to_be_bytes(s: ArkFr) -> [u8; 32] {
        ...
        s.serialize_with_mode(&mut le_buf[..], Compress::No)
            .unwrap();
        let mut be = [0u8; 32];
        for i in 0..32 {
            be[i] = le_buf[31 - i];
    ...
    fn g1_add_matches_arkworks() {
        ...
        assert_eq!(
            got,
            expected_be,
            "g1_add bytes-out mismatch (feature: simd-0284-le={})",
            cfg!(feature = "simd-0284-le")
        );
    ...
    fn g1_mul_matches_arkworks() {
        ...
        assert_eq!(
            got,
            expected_be,
            "g1_mul bytes-out mismatch (feature: simd-0284-le={})",
            cfg!(feature = "simd-0284-le")
        );
    }

    #[test]
    fn g1_decompress_matches_uncompressed_generator() {
        use solana_bn254::compression::prelude::alt_bn128_g1_compress_be;

        let generator = g1_to_be_bytes(G1Affine::generator());
        let compressed = alt_bn128_g1_compress_be(&generator).unwrap();
        let decompressed = onchain::g1_decompress(&compressed).unwrap();
        assert_eq!(decompressed, generator);
    }
}

pub use onchain::{g1_add, g1_decompress, g1_mul, g2_add, g2_mul, keccak256, pairing_check};
