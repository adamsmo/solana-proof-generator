...
pub trait RangeInstructions<F: ScalarField> {
        ...
        F: BigPrimeField,
    {
        ...
        let bcr_hi = self
            .gate()
            .mul_add(ctx, b, Existing(div_hi), Existing(bcr0_hi));
    ...
    fn get_last_bit(
        ...
        let bit_v = F::from(u64::from(a_big.bit(0)));
        let two = F::from(2u64);
        ...
        F: BigPrimeField,
    {
        ...
        let pows = self
            ...
            .take(num_limbs)
            .map(|x| Constant(*x));
        let limb_vals = decompose(num.value(), num_limbs, limb_bits)
            .into_iter()
            .map(Witness);
...
impl<F: ScalarField> RangeChip<F> {
        ...
        F: BigPrimeField,
    {
        ...
        let val_witness = self.gate.mul_add(
            ctx,
            l_witness,
            Constant(self.gate.pow_of_two()[bit]),
            r_witness,
        );
