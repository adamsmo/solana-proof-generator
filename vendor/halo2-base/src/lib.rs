...
/// Witness value stored by the virtual circuit builder.
#[derive(Clone, Copy, Debug)]
pub enum Assigned<F: ff::Field> {
    /// Zero value.
    Zero,
    /// Direct field value.
    Trivial(F),
...
impl<F: ff::Field> Assigned<F> {
    /// Evaluates the stored value in the field.
...
impl<F: ff::Field> From<F> for Assigned<F> {
    fn from(value: F) -> Self {
        Self::Trivial(value)
    }
}

impl<F: ff::Field> From<&Assigned<F>> for Assigned<F> {
...
impl<F: ScalarField> AssignedValue<F> {
    ...
    /// Debug helper for writing negative tests that correctly handles equality constraints.
    ///
    /// Unlike [`debug_prank`](Self::debug_prank) which only modifies a single cell (causing
    /// equality constraints to fail and masking missing gate constraints), `debug_smart_prank`
    /// modifies ALL cells in the same equality chain. This ensures:
    /// - Equality constraints still hold after pranking
    /// - Only gate constraints can detect the invalid value
    ///
    /// `ctx` must be the context containing this cell and all its equality-constrained copies.
    ///
    /// Returns the number of cells pranked.
    ///
    /// # Panics
    /// - If `self.cell` is `None` (witness_gen_only mode)
    /// - If an equivalent cell is in a different context than `ctx`
    pub fn debug_smart_prank(&self, ctx: &mut Context<F>, prank_value: F) -> usize {
        let target = self
            .cell
            .expect("debug_smart_prank requires cell tracking (witness_gen_only must be false)");

        let equivalent_cells = {
            let cm = ctx.copy_manager.lock().unwrap();
            cm.find_equivalent_cells(target)
        };

        let ctx_type_id = ctx.type_id();
        let ctx_id = ctx.id();

        for cell in &equivalent_cells {
            assert!(
                cell.type_id == ctx_type_id && cell.context_id == ctx_id,
                "Equivalent cell {cell:?} is in a different context than ctx ({ctx_type_id}, {ctx_id})."
            );
            ctx.advice[cell.offset] = Assigned::Trivial(prank_value);
        }

        equivalent_cells.len()
    }
}
