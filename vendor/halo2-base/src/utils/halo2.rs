...
use crate::halo2_proofs::{
    halo2curves::bn256::Bn256, plonk::Circuit, poly::kzg::params::ParamsKZG,
};
...
pub type Halo2AssignedCell<F> = AssignedCell<F, F>;

/// Assign advice to physical region.
#[inline(always)]
pub fn raw_assign_advice<F: Field>(
    region: &mut Region<F>,
    ...
    {
        let value = value.map(|a| Into::<Assigned<F>>::into(a).evaluate());
        region
            .assign_advice(
