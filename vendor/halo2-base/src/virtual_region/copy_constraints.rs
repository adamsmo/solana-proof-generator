...
use crate::{Assigned, AssignedValue};
...
impl<F: Field + Ord> CopyConstraintManager<F> {
    ...
    /// Finds all cells in the same equality equivalence class as `target`.
    ///
    /// When "smart pranking" a cell, you need to also prank all equality-constrained copies.
    /// Otherwise, the equality constraint fails first, masking potential missing gate constraints.
    ///
    /// Returns all [`ContextCell`]s transitively connected to `target` through
    /// [`advice_equalities`](Self::advice_equalities), including `target` itself.
    pub fn find_equivalent_cells(&self, target: ContextCell) -> Vec<ContextCell> {
        // Build adjacency list from equality constraints
        let mut adj: HashMap<ContextCell, Vec<ContextCell>> = HashMap::new();
        for &(a, b) in &self.advice_equalities {
            adj.entry(a).or_default().push(b);
            adj.entry(b).or_default().push(a);
        }
        // simple BFS from target to find all transitively equal cells
        ...
        while let Some(cell) = queue.pop_front() {
            if let Some(neighbors) = adj.get(&cell) {
                for &neighbor in neighbors {
                    // add neighbor to the queue if visited does not contain it
    ...
    /// Returns `Some(constant)` if any cell in `target`'s equivalence class is constrained
    /// to a constant value. Returns `None` otherwise.
    ///
    /// Useful to check before smart-pranking: if a cell is constant-constrained, pranking
    /// with a different value will always fail on the constant constraint regardless.
    pub fn is_constant_constrained(&self, target: ContextCell) -> Option<F> {
        let equivalent = self.find_equivalent_cells(target);
        self.constant_equalities
            .iter()
            .find(|(_, cell)| equivalent.contains(cell))
            .map(|(val, _)| *val)
    }
